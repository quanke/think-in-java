...
也就是说，TrashSorter是由一系列指针构成的Vector（系列），而那些指针指向的又是由Trash指针构成的Vector；利用addElement()，可以安装新的TrashSorter，如下所示：
...
(1) TbinList容纳一系列Tbin指针，所以在查找与我们传递给它的Trash对象相符的情况时，sort()能通过Tbin继承。
